...
public class Score {
	...
	public static void main(String[] args) {
		/*
		 * 우리반 모두의 국어 영ㅇ ㅓ수학 사회 과학 오라클 자바 점수를 0~100 까지 랜덤생성해주시고 아래와 같이추력해주세요
		 */

		String[] name = { "강지원", "김동민", "김동희", "김민지", "김용진", "김인혁", "김진용",
				"박범영", "박병규", "배현태", "심민규", "여인욱", "이나정", "이승민", "이영섭", "이윤혜",
				"이지향", "임정혁", "임형묵", "장성보", "정선영", "정회립", "차경석", "최준영", "허지영" };
		...
		for (int i = 0; i < sub_avg.length; i++) {
			sub_avg[i] = sub_sum[i] / (double) scores.length;
		}
		// 출력 하기

		System.out
				.printf("이름\t 국어\t 영어\t 수학\t 사회\t 과학\t ORACLE\t Java\t 합계\t 평균\t 석차\n");
		// for (int i = 0 ; i<scores.length; i++){
		...
		for (int j = 0; j < sub_avg.length; j++) {
			System.out.printf("%.2f\t", sub_avg[j]);
		}
